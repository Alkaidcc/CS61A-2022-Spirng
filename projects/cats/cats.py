...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    result = [para for para in paragraphs if select(para)]
    return result[k] if len(result) > k else ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def is_about(paragraph):
        ...
        return any(word in paragraph for word in topic)
    return is_about
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if typed_words == reference_words:
        return 100.0
    if len(typed_words) == 0 and len(reference_words) != 0 or len(typed_words) != 0 and len(reference_words) == 0:
        return 0.0
    correct = sum(typed_words[i] == reference_words[i] for i in range(min(len(reference_words),len(typed_words))))
...
def fastest_words(match):
    ...
    for col in word_indices:
        for row in player_indices:
            if match["times"][row][col] < match["times"][row_index][col]:
                row_index = row
        words[row_index].append(match["words"][col])
        row_index= 0
    return words
    # END PROBLEM 10
