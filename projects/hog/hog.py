...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    # !!!!Attention!!!! : Make sure you call dice exactly num_rolls times.
    total = 0
    flag = True
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    # if dice 0 times according Hefty Hogs Rule.
    if num_rolls == 0:
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_function(*args):
        total = sum(original_function(*args) for _ in range(total_samples))
